"""
script to train on graph property task
"""
...
class LogMeanSquaredError(MeanSquaredError):
    def compute(self) -> Tensor:
        """Computes mean squared error over state."""
        return torch.log10(_mean_squared_error_compute(self.sum_squared_error, self.total, squared=self.squared))

def main():
    parser = train_utils.args_setup()
    parser.add_argument('--dataset_name', type=str, default="GraphProperty", help='name of dataset')
    parser.add_argument('--task', type=int, default=0, choices=(0, 1, 2), help='name of dataset')
    ...
    train_dataset = GraphPropertyDataset(path,
                                         split='train',
    ...
    val_dataset = GraphPropertyDataset(path,
                                         split='val',
                                         pre_transform=pre_transform,
                                         transform=train_utils.PostTransform(args.wo_node_feature,
                                                                             args.wo_edge_feature,
    ...
    test_dataset = GraphPropertyDataset(path,
                                         split='test',
                                         pre_transform=pre_transform,
                                         transform=train_utils.PostTransform(args.wo_node_feature,
                                                                             args.wo_edge_feature,
    ...
    for i in range(1, args.runs + 1):
        ...
        loss_cri = nn.MSELoss()
        evaluator = LogMeanSquaredError()
        init_encoder = LinearEncoder(2, args.hidden_channels)
