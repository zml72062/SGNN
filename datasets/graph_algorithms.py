...
def is_connected(A: np.ndarray) -> bool:
    r"""Given an adjacency matrix of a graph, return whether it is connected or not.
    Args:
...
def identity(A: np.ndarray,
             F: np.ndarray) -> np.ndarray:
    r"""Return node feature matrix.
    Args:
        A (np.ndarray): The adjacency matrix.
        F (np.ndarray): The node feature matrix.
    """
    return F


def first_neighbours(A: np.ndarray) -> np.ndarray:
    r"""For each node, return the number of nodes reachable in 1 hop.
    Args:
        A (np.ndarray): The adjacency matrix.
    """
...
def second_neighbours(A: np.ndarray) -> np.ndarray:
    r"""For each node, return the number of nodes reachable in no more than 2 hops.
    Args:
        A (np.ndarray): The adjacency matrix.
    """
    A = A > 0.0
...
def kth_neighbours(A: np.ndarray,
                   k: int) -> np.ndarray:
    r"""For each node, return the number of nodes reachable in k hops
    Args:
        A (np.ndarray): The adjacency matrix.
        k (int): The number of hop.
    """
    A = A > 0.0
    R = np.zeros(A.shape)
    for _ in range(k):
...
def map_reduce_neighbourhood(A: np.ndarray,
                             F: np.ndarray,
                             f_reduce: Callable,
                             f_map: Callable = None,
                             hops: int = 1,
                             consider_itself:bool = False) -> np.ndarray:
    r"""For each node, map its neighbourhood with f_map, and reduce it with f_reduce
    Args:
        A (np.ndarray): The adjacency matrix.
        F (np.ndarray): The node feature matrix.
        f_reduce (Callable): Reduction function.
        f_map (Callable): Mapping function.
        hops (int): The number of hop.
        consider_itself (Bool): If true, add node itself.
    """
...
def max_neighbourhood(A: np.ndarray,
                      F: np.ndarray) -> np.ndarray:
    r"""For each node, return the maximum in its neighbourhood.
    Args:
        A (np.ndarray): The adjacency matrix.
        F (np.ndarray): The node feature matrix.
    """
...
def min_neighbourhood(A: np.ndarray,
                      F: np.ndarray) -> np.ndarray:
    r"""For each node, return the minimum in its neighbourhood.
    Args:
        A (np.ndarray): The adjacency matrix.
        F (np.ndarray): The node feature matrix.
    """
...
def std_neighbourhood(A: np.ndarray,
                      F: np.ndarray) -> np.ndarray:
    r"""For each node, return the standard deviation of its neighbourhood.
    Args:
        A (np.ndarray): The adjacency matrix.
        F (np.ndarray): The node feature matrix.
    """
...
def mean_neighbourhood(A: np.ndarray,
                       F: np.ndarray) -> np.ndarray:
    r"""For each node, return the mean of its neighbourhood.
    Args:
        A (np.ndarray): The adjacency matrix.
        F (np.ndarray): The node feature matrix.
    """
...
def local_maxima(A: np.ndarray,
                 F: np.ndarray) -> bool:
    r"""For each node, return whether it is the maximum in its neighbourhood
    Args:
        A (np.ndarray): The adjacency matrix.
        F (np.ndarray): The node feature matrix.
    """
...
def graph_laplacian(A: np.ndarray) -> np.ndarray:
    r"""Return the laplacian of the adjacency matrix.
    Args:
        A (np.ndarray): The adjacency matrix.
    """
    L = (A > 0) * -1
...
def graph_laplacian_features(A: np.ndarray,
                             F: np.ndarray) -> bool:
    r"""Return the laplacian of the adjacency matrix multiplied by the features.
    Args:
        A (np.ndarray): The adjacency matrix.
        F (np.ndarray): The node feature matrix.
    """
